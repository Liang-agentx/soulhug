...
import { Save, FolderOpen, Ghost, Wand2, ArrowLeftRight } from "lucide-react";
...
export function Chat({
  ...
  const toolbarItems: ToolBarItem[] = React.useMemo(
    () => [
      { id: "tree-hole", icon: <Ghost className="w-4 h-4" />, label: "树洞模式 (Tree Hole)", variant: "primary" },
      { id: "empathy", icon: <ArrowLeftRight className="w-4 h-4" />, label: "换位思考 (Empathy Lens)" },
      { id: "skills", icon: <Wand2 className="w-4 h-4" />, label: "能力卡片 (Skills)" },
      { id: "folder", icon: <FolderOpen className="w-4 h-4" />, label: "上传附件 (File)" },
    ],
    []
  );
