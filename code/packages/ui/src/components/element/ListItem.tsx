      ...
      <div ref={ref} className={cn("group relative", className)} {...props}>
        ...
        {!isMobile && (
          <>
            <div
              className={cn(
                "w-full p-3 rounded-xl transition-all duration-300 mb-2 border border-transparent",
                isClickable && "cursor-pointer hover:bg-white/10 hover:-translate-y-0.5 hover:shadow-sm",
                selected && "glass-panel shadow-soulhug-glow scale-[1.02] border-primary/30 !bg-white/20",
            ...
            >
              <div className="flex items-center gap-3 min-w-0 w-full">
                {/* Leading */}
                {leading && <div className="flex-shrink-0 text-primary/80">{leading}</div>}

                {/* Content */}
                <div className="min-w-0 flex-1">
                  <div
                    className={cn(
                      "font-medium truncate text-foreground transition-colors",
                      selected ? "text-primary font-bold" : "text-foreground/80",
                      isCompact ? "text-xs" : "text-sm"
                    )}
                  >
                    {title}
                  </div>
                  {subtitle && <div className="flex items-center gap-1 mt-0.5 text-xs opacity-70">{subtitle}</div>}
                </div>
            ...
            {actions && (
              <div className="absolute right-3 top-1/2 transform -translate-y-1/2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all duration-200">
                {actions}
              </div>
            )}
          </>
