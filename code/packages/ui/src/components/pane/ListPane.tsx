...
export const ListPane = React.forwardRef<HTMLDivElement, ListPaneProps>(
  (
    ...
    return (
      <Sidebar ref={ref} className={cn("flex flex-col bg-transparent", className)}>
        {/* Header */}
        <div className="flex items-center gap-2 px-4 py-4 pb-2">
          ...
          <h2 className="text-lg font-bold text-foreground/90 tracking-tight flex-1 pl-1">{title}</h2>
        </div>
            ...
            {searchable && (
              <div className="flex-1">
                <SearchInput
                  ...
                  className="bg-white/5 border-transparent focus:bg-white/10 transition-all rounded-xl"
                />
              </div>
            )}
            {showNewButton && onNew && (
              <button
                className="p-2 rounded-xl bg-primary/10 text-primary hover:bg-primary/20 hover:scale-105 transition-all flex-shrink-0 shadow-sm"
                onClick={onNew}
